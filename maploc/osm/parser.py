...
from .reader import BIMElement, OSMData, OSMElement, OSMNode, OSMWay

IGNORE_TAGS = {"source", "phone", "entrance", "inscription", "note", "name"}


def parse_bim_element(element: BIMElement): #BIMData
    element_type = element.get_property("type")
    material = element.get_property("material")
    if element_type and material:
        return f"{element_type}:{material}"
    elif element_type:
        return f"general:{element_type}"
    return None


def filter_bim_element(element: BIMElement): #BIMData
    type_ = element.get_property("type")
    function = element.get_property("function")
    return type_ in ['structural', 'mep', 'finishing'] and function in ['load-bearing', 'HVAC', 'aesthetic']


class BimPatterns: #BIMData
    structural = {
    ...
    mep = {
        "flowsegment": "mep:flowsegment",
        "flowfitting": "mep:flowfitting",
        "duct": "mep:duct",
        "pipe": "mep:pipe",
        "wire": "mep:wire"
    }
    finishing = {
        "window": "finishing:window",
        "door": "finishing:door"
    }

    all = {**structural, **mep, **finishing}

class BIMGroups: #BIMData
    structural = BimPatterns.structural
    mep = BimPatterns.mep
    finishing = BimPatterns.finishing
    all = {**structural, **mep, **finishing}

def group_bim_elements(bim_data):
    elem2group = {"structural": {}, "mep": {}, "finishing": {}}
    for element in filter(filter_bim_element, bim_data.values()):  # Assuming bim_data is a dictionary of BIMElement objects
        label = parse_bim_element(element)
        if label is None:
            continue
        group = match_to_group(label, BimPatterns.all)
        if group is None:
            continue
        elem2group[group][element.id_] = label

    return elem2group


#Original Codes below
def match_to_group(label, patterns): #GeneralUse
